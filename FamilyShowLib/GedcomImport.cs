...
public class GedcomImport
{
    ...
    private void UpdatePeopleIDs()
    {
        foreach (Person p in _people)
        {
            ...
            if (!string.IsNullOrEmpty(p.Note))
            {
                ...
                string FileName =
                    new StringBuilder(
                        App.ReplaceEncodedCharacters(p.FullName + "(" + p.Id + ")")).Append(".rtf").ToString();
        ...
        foreach (XmlNode node in list)
        {
            ...
            foreach (string child in children)
            {
                ...
                if (husbandPerson != null && wifePerson != null & childPerson != null)
                {
                    ...
                    {
                        if (s != childPerson &&
                            wifeModifier == ParentChildModifier.Natural &&
                            husbandModifier == ParentChildModifier.Natural)
                        {
    ...
    private static void ImportDeath(Person person, XmlNode node, XmlDocument doc)
    {
        ...
        // if no deat tag and no buial tagcheck cremation tag
        if (node.SelectSingleNode("DEAT") == null && node.SelectSingleNode("BURI") == null)
        {
        ...
        {
            // make an assumption that anyone without a death date and over 90 is dead.
            // This leads to far less people imported as living when then are dead
            // since GEDCOM does not have an IsLiving field.
    ...
    private static void ImportNames(Person person, XmlNode node)
    {
        ...
        foreach (XmlNode nameNode in nameNodes)
        {
            ...
            Name name = new(givenName, surname, prefix, suffix, surnamePrefix, type, isPrimary);
