...
public static class RelationshipHelper
{
    ...
    /// </summary>
    public static void AddExistingChild(
        PeopleCollection family,
        Person person,
        Person child,
        ParentChildModifier modifier)
    ...
    public static void AddParent(PeopleCollection family, Person person, Person parent)
    {
        ...
        // if a person already has a parent with the same gender,
        // then add as an adopted parent (could be foster)
        if (i >= 1)
        {
            family.Add(parent);
    ...
    /// In the case of existing people, do not hook up other siblings etc.  Allow the user to do this manually.
    /// </summary>
    public static void AddExistingParent(
        PeopleCollection family,
        Person person,
        Person parent,
        ParentChildModifier modifier)
    ...
    private static List<Person> GetChildren(ParentSet parentSet)
    {
        ...
        List<Person> firstParentChildren = [.. parentSet.FirstParent.Children];
        List<Person> secondParentChildren = [.. parentSet.SecondParent.Children];
    ...
    /// In the case of existing people, do not hook up other siblings etc.  Allow the user to do this manually.
    /// </summary>
    public static void AddExistingSpouse(
        PeopleCollection family,
        Person person,
        Person spouse,
        SpouseModifier modifier)
    {
        // Assume the spouse's gender based on the counterpart of the person's gender
        if (person.Gender == Gender.Male)
        {
    ...
    /// </summary>
    public static void UpdateMarriageCitationActualText(
        Person person,
        Person spouse,
        string marriageCitationActualText)
    {
        foreach (Relationship relationship in person.Relationships)
        {
            if (relationship.RelationshipType == RelationshipType.Spouse && relationship.RelationTo.Equals(spouse))
    ...
    public static void RemoveSiblingRelationships(Person person, Person parent)
    {
        foreach (Person p in parent.Children)
        {
            ...
            {
                if (relationship.RelationshipType == RelationshipType.Sibling &&
                    relationship.RelationTo.Equals(p))
                {
