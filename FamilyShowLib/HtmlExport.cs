 ...
 * 
 * tw.WriteLine(FamilyShowLib.Properties.Resources.Tree + " <b>" +
 * Path.GetFileNameWithoutExtension(familyxFileName) + "</b><br/>");
...
public class HtmlExport
{
        ...
        bool sourcesbool)
    {
        ...
        if (!string.IsNullOrEmpty(familyxFileName))
        {
            _tw.WriteLine(
                "<b>" +
                Path.GetFileNameWithoutExtension(familyxFileName) + " "
                + Properties.Resources.FamilyTree + "</b><br/>");
        }
        ...
        foreach (Person p in peopleCollection)
        {
            ...
            if (p.IsLiving == true && privacy == true && p.Restriction != Restriction.Private) //quick privacy option
            {
                _tw.WriteLine(
                    "<tr id=\"id_" +
                    p.Id + "\" class=\"person\"><td>"
                    + Properties.Resources.Living + "</td><td>" +
                    p.LastName + "</td><td></td><td></td><td></td><td></td><td>" +
            ...
            else if (p.Restriction == Restriction.Private) //a private record should not be exported
            {
                _tw.WriteLine(
                    "<tr id=\"id_" +
                    p.Id + "\" class=\"person\"><td>" +
                    Properties.Resources.PrivateRecord + "</td><td></td><td></td>" +
        ...
        bool sourcesbool)
    {
        ...
        // add the current person, their parents, spouses, previous spouses,
        // children and siblings to the export people collection
        ...
        {
            _tw.WriteLine(
                "<b>" + Path.GetFileNameWithoutExtension(familyxFileName) + " "
                + Properties.Resources.FamilyTree + "</b><br/>");
        }
        ...
        foreach (Person p in pc)
        {
            ...
            foreach (Relationship rel in peopleCollection.Current.Relationships)
            {
                if (rel.RelationTo == p)
                {
                    ...
                    // quick privacy option
                    if (p.IsLiving == true && privacy == true && p.Restriction != Restriction.Private)
                    {
                        _tw.WriteLine(
                            "<tr id=\"id_" +
                            p.Id + "\" class=\"person\"><td>" +
                            relationship + "</td><td>" +
                            Properties.Resources.Living + "</td><td>" +
                            p.LastName + "</td><td></td><td></td><td></td><td>" +
                    ...
                    else if (p.Restriction == Restriction.Private) //a private record should not be exported
                    {
                        _tw.WriteLine(
                            "<tr id=\"id_" +
                            p.Id + "\" class=\"person\"><td>" +
                            Properties.Resources.PrivateRecord + "</td><td></td>" +
                            "<td></td><td></td><td></td><td></td><td></td><td>" +
                            "</td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td></tr>");
                    }
                    else
    ...
    public void ExportCurrent(
        ...
        SourceCollection sourceCollection,
        RepositoryCollection repositoryCollection,
        string htmlFilePath, string familyxFileName,
        bool privacy,
        bool sourcesbool)
    {
        PeopleCollection pc = [];
        ...
        foreach (Person p in pc)
        {
            ...
            if (p.IsLiving == true && privacy == true && p.Restriction != Restriction.Private) //quick privacy option
            {
                _tw.WriteLine(
                    ...
                    p.Id + "\" class=\"person\"><td>" +
                    Properties.Resources.Living + "</td><td>" +
                    p.LastName + "</td><td></td><td></td><td></td><td></td><td></td>" +
        ...
        bool sourcesbool)
    {
        ...
        // add the current person, their spouses and siblings to the export people
        // collection and then repeat for each specified generation
    ...
    /// </summary>
    public void ExportEventsByDecade(
        PeopleCollection peopleCollection,
        string htmlFilePath,
        string familyxFileName,
        bool privacy,
        ...
        int endYear)
    {
        ...
        _tw.WriteLine("<h2>" + Properties.Resources.FamilyShow + "</h2>");
        _tw.WriteLine(
            "<i>" + Properties.Resources.Events + " " +
            startYear.ToString() + " - " +
            endYear.ToString() + "</i><br/><br/>");
        if (!string.IsNullOrEmpty(familyxFileName))
        {
            _tw.WriteLine(
                "<b>" +
        ...
        for (int i = startYear; i <= endYear; i += 10)
        {
            ...
            if (i < DateTime.Now.Year) // Only export events which have happened.
            {
                ...
                _tw.WriteLine(
                    "<p class=\"decade\"><span class=\"tick\">&#8212;</span><b>" +
                    i.ToString() + "-" +
                    ii.ToString() + "</b></p>");
                        ...
                        if (year >= i && year <= ii)
                        {
                            if (!string.IsNullOrEmpty(p.BirthPlace))
                            {
                                _tw.WriteLine(
                                    "<p class=\"event\"><b>" +
                                    p.FullName + "</b> " +
                                    Properties.Resources.WasBornOn + " " +
                                    DateWrapper.Format(p.BirthDate) + " " +
                                    Properties.Resources.In + " " +
                                    p.BirthPlace + ".</p>");
                            }
                            else
                            {
                                _tw.WriteLine(
                                    ...
                                    p.FullName + "</b> " +
                                    Properties.Resources.WasBornOn + " " +
                                    DateWrapper.Format(p.BirthDate) + ".</p>");
                            }
                        }
                    }
                        ...
                        if (year >= i && year <= ii)
                        {
                            ...
                            else
                            {
                                _tw.WriteLine(
                                    ...
                                    p.FullName + "</b> " +
                                    Properties.Resources.DiedOn + " " +
                                    DateWrapper.Format(p.DeathDate) + ".</p>");
                            }
                        }
                        year = 0;
                    }
                        ...
                        if (year >= i && year <= ii)
                        {
                            if (!string.IsNullOrEmpty(p.BurialPlace))
                            {
                                _tw.WriteLine(
                                    "<p class=\"event\"><b>" +
                                    p.FullName + "</b> " +
                                    Properties.Resources.WasBuriedOn + " " +
                            ...
                            else
                            {
                                _tw.WriteLine(
                                    ...
                                    p.FullName + "</b> " +
                                    Properties.Resources.WasBuriedOn + " " +
                                    DateWrapper.Format(p.BurialDate) + ".</p>");
                            }
                        }
                        year = 0;
                    }
                        ...
                        if (year >= i && year <= ii)
                        {
                            if (!string.IsNullOrEmpty(p.CremationPlace))
                            {
                                _tw.WriteLine(
                                    "<p class=\"event\"><b>" +
                                    p.FullName + "</b> " +
                                    Properties.Resources.WasCrematedOn + " " +
                            ...
                            else
                            {
                                _tw.WriteLine(
                                    ...
                                    p.FullName + "</b> " +
                                    Properties.Resources.WasCrematedOn + " " +
                                    DateWrapper.Format(p.CremationDate) + ".</p>");
                            }
                        }
                        year = 0;
                    }
    ...
    private static string dataWithNote(Person p, string[,] sourceArray, string relationship = null)
    {
        ...
        return "<tr id=\"id_" +
            p.Id + "\" class=\"personhighlight\">" +
            relationshipOutput + "<td>" +
            p.FirstName + "</td><td>" +
            p.LastName + "</td><td>" +
            p.Age + "</td><td>" +
            p.BirthDateDescriptor + " " + DateWrapper.Format(p.BirthDate) + sourceArray[0, 0] + "</td><td>" +
            p.BirthPlace + sourceArray[0, 0] + "</td><td>" +
            p.DeathDateDescriptor + " " + DateWrapper.Format(p.DeathDate) + sourceArray[0, 1] + "</td><td>" +
            ...
            p.BurialDateDescriptor + " " + DateWrapper.Format(p.BurialDate) + sourceArray[0, 5] + "</td><td>" +
            p.CremationPlace + sourceArray[0, 6] + "</td><td>" +
            p.CremationDateDescriptor + " " + DateWrapper.Format(p.CremationDate) + sourceArray[0, 6] + "</td><td>" +
            "<p class=\"notelink\">[<a href=\"javascript:showhide('id_" +
            p.Id + "')\">Note</a>]</p></td></tr><tr id=\"note_id_" +
            p.Id + "\" class=\"noteshown\">" +
            colspan + "<b>" +
            Properties.Resources.Note + "</b>: <pre>" +
            p.Note + "</pre></td></tr>";
    }
    ...
    private static string dataWithoutNote(Person p, string[,] sourceArray, string relationship = null)
    {
        ...
        return "<tr id=\"id_" +
            ...
            p.BurialDateDescriptor + " " + DateWrapper.Format(p.BurialDate) + sourceArray[0, 5] + "</td><td>" +
            p.CremationPlace + sourceArray[0, 6] + "</td><td>" +
            p.CremationDateDescriptor + " " + DateWrapper.Format(p.CremationDate) + sourceArray[0, 6] +
            "</td><td></td></tr>";
    }
    ...
    private static string[,] Sources(Person p)
    {
        ...
        if (!string.IsNullOrEmpty(p.BirthCitation) && !string.IsNullOrEmpty(p.BirthSource))
        {
            sourceArray[0, 0] =
                "<br/><br/><i>" +
                p.BirthCitation + "</i> [<a href=\"#" +
                p.BirthSource + "\">" +
                p.BirthSource + "</a>]";
            if (!string.IsNullOrEmpty(p.BirthLink) &&
                (p.BirthLink.StartsWith("www.") || p.BirthLink.StartsWith("http://")))
            {
        ...
        if (!string.IsNullOrEmpty(p.DeathCitation) && !string.IsNullOrEmpty(p.DeathSource))
        {
            sourceArray[0, 1] =
                "<br/><br/><i>" +
                p.DeathCitation + "</i> [<a href=\"#" +
                p.DeathSource + "\">" +
                p.DeathSource + "</a>]";
            if (!string.IsNullOrEmpty(p.DeathLink) &&
                (p.DeathLink.StartsWith("www.") || p.DeathLink.StartsWith("http://")))
            {
        ...
        if (!string.IsNullOrEmpty(p.OccupationCitation) && !string.IsNullOrEmpty(p.OccupationSource))
        {
            sourceArray[0, 2] =
                "<br/><br/><i>" +
                p.OccupationCitation +
                "</i> [<a href=\"#" + p.OccupationSource +
                "\">" + p.OccupationSource + "</a>]";
            if (!string.IsNullOrEmpty(p.OccupationLink) &&
                (p.OccupationLink.StartsWith("www.") || p.OccupationLink.StartsWith("http://")))
            {
        ...
        if (!string.IsNullOrEmpty(p.EducationCitation) && !string.IsNullOrEmpty(p.EducationSource))
        {
            sourceArray[0, 3] =
                "<br/><br/><i>" +
                p.EducationCitation + "</i> [<a href=\"#" +
                p.EducationSource + "\">" +
                p.EducationSource + "</a>]";
            if (!string.IsNullOrEmpty(p.EducationLink) &&
                (p.EducationLink.StartsWith("www.") || p.EducationLink.StartsWith("http://")))
            {
        ...
        if (!string.IsNullOrEmpty(p.ReligionCitation) && !string.IsNullOrEmpty(p.ReligionSource))
        {
            sourceArray[0, 4] =
                "<br/><br/><i>" +
                p.ReligionCitation + "</i> [<a href=\"#" +
                p.ReligionSource + "\">" +
                p.ReligionSource + "</a>]";
            if (!string.IsNullOrEmpty(p.ReligionLink) &&
                (p.ReligionLink.StartsWith("www.") || p.ReligionLink.StartsWith("http://")))
            {
        ...
        if (!string.IsNullOrEmpty(p.BurialCitation) && !string.IsNullOrEmpty(p.BurialSource))
        {
            sourceArray[0, 5] =
                "<br/><br/><i>" +
                p.BurialCitation + "</i> [<a href=\"#" +
                p.BurialSource + "\">" +
                p.BurialSource + "</a>]";
            if (!string.IsNullOrEmpty(p.BurialLink) &&
                (p.BurialLink.StartsWith("www.") || p.BurialLink.StartsWith("http://")))
            {
        ...
        if (!string.IsNullOrEmpty(p.CremationCitation) && !string.IsNullOrEmpty(p.CremationSource))
        {
            sourceArray[0, 6] =
                "<br/><br/><i>" +
                p.CremationCitation + "</i> [<a href=\"#" +
                p.CremationSource + "\">" +
                p.CremationSource + "</a>]";
            if (!string.IsNullOrEmpty(p.CremationLink) &&
                (p.CremationLink.StartsWith("www.") || p.CremationLink.StartsWith("http://")))
            {
    ...
    private static string Buttons()
    {
        ...
        return "<input type=\"button\" onclick=\"showhideall('hide_all')\" value=\"" +
            Properties.Resources.HideNotes + "\" />\n" +
           "<input type=\"button\" onclick=\"showhideall('show_all')\" value=\"" +
           Properties.Resources.ShowNotes + "\" />\n" +
           "<noscript>\n" +
    ...
    /// <summary>
    /// Write the tw.WriteLine(FamilyShowLib.Properties.Resources.Tree + " <b>" +
    /// Path.GetFileNameWithoutExtension(familyxFileName) + "</b><br/>");s
    /// </summary>
    ...
    private static string Footer()
    {
        ...
        return "</table><br/><p><i>" +
            ...
            versionlabel + " " + Properties.Resources.On + " " +
            date + "</i></p></body></html>";
    }
    ...
    private static string CurrentPersonString(Person p, bool privacy)
    {
        ...
        if (p.IsLiving == true && privacy == true && p.Restriction != Restriction.Private) //quick privacy option
        {
            CurrentPerson = Properties.Resources.CurrentPerson + " <b>" +
                Properties.Resources.Living + " " + p.LastName + "</b><br/>";
        }
        else if (p.Restriction == Restriction.Private) //a private record should not be exported
        {
            CurrentPerson = Properties.Resources.CurrentPerson + " <b>" +
                Properties.Resources.PrivateRecord + "</b><br/>";
        }
        else
        {
            CurrentPerson = Properties.Resources.CurrentPerson + " <b>" +
