...
public class PersonTest
{
    ...
    public void Age_BirthdayOccurredThisYear_ReturnsCorrectAge()
    {
        ...
        Person person = new()
        {
            BirthDate = new(2000, DateTime.Now.Month, 1),
        };
      ...
      )
    {
        using (new AnotherCulture(culture))
        {
            ...
            Assert.Equal(
                birthDate.ToString(DateTimeFormatInfo.CurrentInfo.MonthDayPattern, CultureInfo.CurrentCulture),
                person.BirthMonthAndDay);
            Assert.Equal(
                $"Born {birthDate.ToString(DateTimeFormatInfo.CurrentInfo.ShortDatePattern, CultureInfo.CurrentCulture)}",
                person.BirthDateAndPlace);
        }
    }
        ...
        using (AnotherCulture.UnitedStates())
        {
            ...
            Assert.Equal(
                $"Born {birthDate.ToString(
                    ...
                    CultureInfo.CurrentCulture)}, {birthPlace}",
                person.BirthDateAndPlace);
        }
    }
