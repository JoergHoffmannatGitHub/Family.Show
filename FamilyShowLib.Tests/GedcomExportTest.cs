...
public class GedcomExportTest
{
    ...
    public void ExportSummaryTest(
      ...
      [CombinatorialValues(
        "en-US", "en-GB", "it-IT", "es-ES", "fr-FR", "de-DE", "somthing else")] string languageOfText)
    {
        using (AnotherCulture.UnitedStates())
        {
            // Arrange
            string gedcomFilePath = @"C:\SomePath\" + filename;
            string familyxFilePath = string.IsNullOrEmpty(nameOfSourceDate) ? string.Empty : @"C:\SomePath\" +
                nameOfSourceDate;
            ...
            Assert.Equal(
                !string.IsNullOrEmpty(nameOfSourceDate),
                actual.Contains(FormatLine(2, "DATA", nameOfSourceDate)));
            ...
            Assert.Contains(
                FormatLine(1, "LANG", gedcomExport._culturLanguageIdMap.GetValueOrDefault(languageOfText, "English")),
                actual);
        }
    }
      ...
      string source)
    {
        ...
        gedcomExport._sources.Add(
            new Source(source, "sourceName", "sourceAuthor", "sourcePublisher", "sourceNote", "sourceRepository"));
        using MemoryStream memoryStream = new();
        gedcomExport._writer = new(memoryStream);

        // Act
        gedcomExport.ExportEvent(
            tag,
            tagDescription,
            descriptor,
            date,
