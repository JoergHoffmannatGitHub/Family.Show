...
public class PlacesExportTest
{
    ...
    public void ExportPlaces_EmptyPeopleCollection_ReturnsNoPlaces()
    {
        using (AnotherCulture.UnitedStates())
        {
            ...
            // Act
            string[] result =
                PlacesExport.ExportPlaces(
                    peopleCollection,
                    fileName,
                    false,
                    ExportPlacesType.Places,
                    true,
                    true,
                    true,
                    true,
    ...
    public void ExportPlaces_WithExportPlacesTypes_ExportsCorrectly(
      [CombinatorialValues(
        ExportPlacesType.Places,
        ExportPlacesType.Times,
        ExportPlacesType.Lifespans)] ExportPlacesType exportPlacesType
      )
    {
        using (AnotherCulture.UnitedStates())
        {
      ...
      )
    {
        using (AnotherCulture.UnitedStates())
        {
            ...
            // Assert
            Assert.Contains(
                withBurialDate ? "1 " + Properties.Resources.PlacesExported : Properties.Resources.NoPlaces,
                result[0]);
      ...
      )
    {
        using (AnotherCulture.UnitedStates())
        {
            ...
            // Assert
            Assert.Contains(
                withCremationDate ? "1 " + Properties.Resources.PlacesExported : Properties.Resources.NoPlaces,
                result[0]);
