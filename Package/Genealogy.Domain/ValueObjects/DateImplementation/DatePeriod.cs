...
internal partial class DatePeriod : IDate, IEquatable<IDate>
{
  ...
  /// <param name="result">When this method returns, contains the <see cref="DatePeriod"/> value equivalent to the
  /// date contained in <paramref name="date"/>, if the conversion succeeded, or <c>null</c> if the conversion failed.</param>
  /// <returns><c>true</c> if the <paramref name="date"/> parameter was converted successfully; otherwise, <c>false</c>.</returns>
  ...
  public DatePeriod(string date)
  {
    ...
    if (date.Length < 7)
    {
      throw new GenealogyException($"Invalid date string: {date} too short to contain an period prefix and a date.");
    }
    ...
    {
      throw new GenealogyException($"Invalid date: {date} must contain at least one of the words 'FROM' or 'TO'.");
    }
  }
