...
internal partial class DateRange : IDate, IEquatable<IDate>
{
  ...
  /// <param name="result">When this method returns, contains the parsed <see cref="DateRange"/> object if parsing succeeded; otherwise, <c>null</c>.</param>
  ...
  /// <exception cref="GenealogyException">Thrown if <paramref name="date"/> is less than 8 characters or has an invalid format.</exception>
  public DateRange(string date)
  {
    ArgumentNullException.ThrowIfNull(date);
    if (date.Length < 8)
    {
      throw new GenealogyException($"Invalid date string: {date} too short to contain an rang prefix and a date.");
    }
