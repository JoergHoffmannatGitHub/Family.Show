...
public partial class PersonInfo : UserControl
{
    ...
    private void SaveStoryButton_Click(object sender, RoutedEventArgs e)
    {
        ...
        if (person != null)
        {
            ...
            string storyFileName = new StringBuilder(
                App.ReplaceEncodedCharacters(person.FullName + "(" + person.Id + ")")).Append(".rtf").ToString();
    ...
    private void Link_Click(object sender, RoutedEventArgs e)
    {
        ...
        if (photoCount > 0)
        {
            ...
            {
                // only link files which are in the temp directory
                if (File.Exists(Path.Combine(photoLocation, Path.GetFileName(dialog.FileName))))
                {
                    if (App.IsPhotoFileSupported(Path.GetFileName(dialog.FileName)))
                    {
                        ...
                        {
                            if (p.RelativePath.ToString() ==
                                Path.Combine(Photo.PhotosFolderName, Path.GetFileName(dialog.FileName)))
                            {
                                i++;
                            }
                        }
        ...
        if (person.Restriction != Restriction.Locked)
        {
            ...
            foreach (string fileName in fileNames)
            {
                ...
                if (App.IsPhotoFileSupported(fileName))
                {
                    ...
                    if (person.Photos.Count == 0)
                    {
                        // turned this off as I found it became more a hinderance than a help, especially
                        // when dragging mulitple files, I may not want the first file to be the primary.
                        //  photo.IsAvatar = true;  
                        PhotosListBox.SelectedIndex = 0;
                    }
