        ...
        internal string DateInformation
        {
            get
            {
                ...
                if (birthdate != null && deathDate != null)
                {
                    ...
                    // change the displayed age when the time slider is altered
                    int age = (_displayYear >= deathDate.Year) ?
                        _person.Age.Value : _person.Age.Value - (deathDate.Year - (int)_displayYear);

                    return string.Format(CultureInfo.CurrentUICulture,
                        "{0}{1} - {2}{3} | {4}",
                        _person.BirthDateDescriptor,
                        birthdate.Year,
                        _person.DeathDateDescriptor,
                        deathDate.Year,
                        Math.Max(0, age));
                }
            ...
            switch (_type)
            {
                ...
                case NodeType.Spouse:
                    if (_person.Parents.Count > 0 ||
                        ...
                        // 1 because if this is a spouse node and they only have 1 previous spouse,
                        // then that spouse must be visible so there is no need to show the indicator
                        _person.PreviousSpouses.Count > 1)
                    {
                        show = true;
                    }
